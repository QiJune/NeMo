...
gpt_model = MegatronGPTModel(cfg=cfg, trainer=trainer)
gpt_model.eval()
...
assert sum([
    id_list == true_id_list for id_list, true_id_list in zip(ids, true_ids)
]) == 4
...
masks_and_position_ids = [
    get_ltor_masks_and_position_ids(id_tensor, gpt_model.tokenizer.eos_id,
                                    False, False, False)
...
with torch.no_grad():
    for tokens, attn_mask_and_pos_ids in zip(id_tensors,
                                             masks_and_position_ids):
        attn_mask, _, pos_ids = attn_mask_and_pos_ids
        assert tokens.shape == pos_ids.shape
        assert attn_mask.shape[2] == attn_mask.shape[3] == tokens.shape[
            1] == pos_ids.shape[1]
